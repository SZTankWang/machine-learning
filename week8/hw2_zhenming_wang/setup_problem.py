...
# get a linear combination of basis step function
...
# generate data using the given target_fn (obtained from 'get_target_and_featurizer')
def generate_data(target_fn, n=1000, noise_scale=.25, tdof=6):
    # Construct dataset
    x = np.sort(np.random.rand(n)) # alternatively we can chooses uniformly from [0,1)
    y_target = target_fn(x)
    y = y_target + noise_scale * np.random.standard_t(tdof,n)
    return x, y


# split data to train set and testing set
...
def main():
    ...
    if GENERATE_PROBLEM:
        ...
        x_train, y_train, x_val, y_val, target_fn, coefs_true, featurize = generate_problem(
            n=n, num_basis_fns=num_basis_fns, num_nonzero=num_nonzero, noise_scale=noise_scale,
